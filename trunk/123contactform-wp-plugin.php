...
function w123cf_widget_text_filter( $content ) {
    ...
    while ($ready==false)
    {
        ...
        if (strpos($tosearch, '[123-contact-form-eu') !== false) {
            $i = strpos($tosearch, "[123-contact-form-eu ");
            $domain = 'eu.123contactform.com';
        }
		...
		$pos_custom_controls=strpos($tosearch,"control",$i+19);
	        if(($pos_custom_controls > $i) &&($pos_custom_controls < $j))
                ...
                if (strpos($tosearch, '[123-contact-form-eu') !== false)
                    $id = substr($tosearch, $i+22, $j-$i-22);
                else
	        ...
	        if (is_numeric($id))
				...
				$formcode = "<script type=\"text/javascript\" defer src=\"//$domain/embed/$id.js\" data-role=\"form\" data-custom-vars=\"$customVars2\"></script>";
				
				$tosearch=str_replace($toreplace, $formcode, $tosearch);
				 				
				if ( is_callable('curl_init') ) {					
					$curl = curl_init("http://"+$domain+"/embedded-link/".$id.".txt");
				...
				elseif ( is_callable('file_get_contents') ) {
					$linkcode=file_get_contents("http://".$domain."/embedded-link/".$id.".txt");
					}
