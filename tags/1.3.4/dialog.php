...
function request($api_key) {
        ...
        if (strpos($api_key, 'EU.') !== false) {
            $url = "http://eu.123contactform.com/wp_dispatcher.php";
        }
...
<html>
    ...
    function contact123(shortcode,api_key) {
        ...
        code = '[123-contact-form i'+shortcode+']';

        if(api_key != 'undefined') {
            if (api_key.indexOf('EU.') > -1) {
                code = '[123-contact-form-eu i' + shortcode + ']';
            }
        }
	...
	if (isset($_POST["apikey"]) && $_POST["apikey"]) {    
		...
		?>
		<p>Please enter your 123ContactForm API Key.<br/> If you don't know it, <a href="http://www.123contactform.com/index.php?p=login" target="_new">login</a> to 123ContactForm, then go to My Account.</p>
